...
let targetX=0.025,targetY=-0.13,targetZoom=1.18,rotationX=targetX,rotationY=targetY;
...
async function init(){
 ...
 document.title=config.title||'闪卡';
 renderer=new THREE.WebGLRenderer({antialias:true,alpha:false,preserveDrawingBuffer:true,powerPreference:'high-performance'});renderer.setClearColor(0x24262a,1);renderer.setPixelRatio(Math.min(devicePixelRatio,1.75));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;stage.append(renderer.domElement);
 ...
 setupControls();new ResizeObserver(resize).observe(stage);resize();loading.remove();setAuto(true);
...
function reset(){targetX=.025;targetY=-.13;targetZoom=1.18;flipped=false;setAuto(false);resize();}
function flip(){flipped=!flipped;setAuto(false);targetY=flipped?Math.PI:0;targetX=0;}
function setupControls(){
 ...
 stage.addEventListener('wheel',e=>{e.preventDefault();targetZoom=THREE.MathUtils.clamp(targetZoom-e.deltaY*.001,.82,1.35);resize();},{passive:false});
...
function animate(now){const dt=Math.min((now-lastTime)/1000,.1)||0;lastTime=now;if(!document.hidden)elapsed+=dt;if(auto){targetY=Math.sin(elapsed*.9)*.55;targetX=Math.sin(elapsed*.5)*.08;}
 const ease=1-Math.exp(-dt*8);rotationX+=(targetX-rotationX)*ease;rotationY+=(targetY-rotationY)*ease;root.rotation.set(rotationX,rotationY,0);root.updateMatrixWorld(true);
